...
class DataExtractor():
        ...
        if type_code == gdb.TYPE_CODE_INT:
            ...
            try:
                ...
                result = self.extract_value(
                    deref, name, current_depth=current_depth+1)
    ...
    def extract_std_vector(self, var, current_depth):
        ...
        try:
            ...
            for i in range(min(size, 100)):  # Limit to 100 elements
                try:
                    elem = start[i]
                    elements.append(self.extract_value(
                        elem, f"[{i}]", current_depth=current_depth+1))
                except:
                    elements.append(
                        {'type': 'error', 'message': 'Cannot access'})
    ...
    def extract_std_list(self, var, current_depth):
        ...
        try:
            # Get node pointer
            node = var['_M_impl']['_M_node'] if '_M_impl' in str(
                var.type) else var['_M_node']
    ...
    def extract_std_map(self, var, current_depth):
        ...
        try:
            ...
            # Get node count
            node_count = int(impl['_M_node_count']
                             ) if '_M_node_count' in str(impl.type) else 0

            # Get header node
            header = impl['_M_header'] if '_M_header' in str(
                impl.type) else None
    ...
    def extract_std_string(self, var):
        ...
        try:
            ...
            try:
                impl = var['_M_dataplus'] if '_M_dataplus' in str(
                    var.type) else var
    ...
    def extract_std_pair(self, var, current_depth):
        """Extract std::pair"""
        try:
            first = self.extract_value(
                var['first'], 'first', current_depth=current_depth+1)
            second = self.extract_value(
                var['second'], 'second', current_depth=current_depth+1)
    ...
    def extract_array(self, var, var_type, current_depth):
        ...
        try:
            ...
            for i in range(min(size, 100)):  # Limit to 100
                elem = self.extract_value(
                    var[i], f"[{i}]", current_depth=current_depth+1)
    ...
    def extract_pointer(self, var, var_type, name, current_depth):
        ...
        try:
            ...
            result['dereferenced'] = {
                'type': 'error', 'message': 'Cannot dereference'}
...
recorder.set_watch_variables(["arr"])
