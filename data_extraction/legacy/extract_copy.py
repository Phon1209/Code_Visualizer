...
class DataExtractor(gdb.Command):
    """Extract data structures to JSON for visualization"""
    
    def __init__(self):
        super(DataExtractor, self).__init__("extract", gdb.COMMAND_DATA)
    
    def invoke(self, arg, from_tty):
        args = gdb.string_to_argv(arg)
        if not args:
            print("Usage: extract <variable_name> [output_file.json]")
            return
        
        var_name = args[0]
        output_file = args[1] if len(args) > 1 else "data_snapshot.json"
